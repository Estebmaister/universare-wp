...
/**
 * Create Reflexiones quoter page at /reflexiones once (idempotent).
 */
function universare_ensure_reflexiones_page(): void {
	if ( ! function_exists( 'wp_insert_post' ) ) {
		return;
	}

	$template = 'page-templates/reflexiones.php';
	$existing = get_page_by_path( 'reflexiones', OBJECT, 'page' );
	...
	delete_option( 'universare_reflexiones_page_created' );

	$page_id = wp_insert_post(
		array(
			'post_title'   => 'Libros y reflexiones',
			'post_name'    => 'reflexiones',
			'post_status'  => 'publish',
			'post_type'    => 'page',
			'post_content' => '',
		),
